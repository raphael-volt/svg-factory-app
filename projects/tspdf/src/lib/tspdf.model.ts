...
export class LayoutUtils {
    static getLayoutSizes(name: LayoutNames, orientation: LayoutOrientation): Sizes | undefined {
        console.log("getLayoutSizes", name, orientation)
        if (!(<Object>layoutSizes).hasOwnProperty(name) || (orientation != "landscape" && orientation != "portrait"))
            return undefined
        const sizes = layoutSizes[name].slice() as Sizes
        if (orientation == 'landscape')
            sizes.reverse()
        return sizes
    }

    static getLayoutName(layout: [number, number]): string {
    ...
    private static SIZE_RATIO = NaN

    private static getSizeRatio(): number {
        if (!isNaN(LayoutUtils.SIZE_RATIO))
            return LayoutUtils.SIZE_RATIO
        LayoutUtils.SIZE_RATIO = 297 / LayoutUtils.getLayoutSizes('A4', 'portrait')[1]
        return LayoutUtils.SIZE_RATIO
    }

    static px2mm(px: number): number {
        return px * LayoutUtils.getSizeRatio()
    }
    static mm2px(mm: number): number {
        return mm / LayoutUtils.getSizeRatio()
    }
}
export {
    ...
    PDFWrapper, TextOptions, Margins, LayoutNames,
