 ...
 * - getPath('', 'prop') => 'prop'
 */
const getPath = (path: string, prop: string): string => {
    if (path.length !== 0) return `${path}.${prop}`;
    else return prop;
}
export class Watcher {

    create(target, validator: Validator) { 
        return this._create(target, validator, '', '')
    }
    private _create(target, validator: Validator, path: string, lastInPath: string) {
        // Keeps track of the proxies we've already made so that we don't have to recreate any.
        let proxies: { [prop: string]: any } = {};
        const _create = this._create
        let proxyHandler = {
            _create:_create,
            get: (target, prop) => {
                // Special properties
                ...
                if (typeof value === 'function') {
                    return function (...args) {
                        ...
                        {
                            let isp = this.__isProxy
                            let target = isp ? this.__target:this
                ...
                // Objects
                    ...
                    if (existingProxy && existingProxy.__target === value) {
                        ...
                        let proxy = this._create(value, validator, getPath(path, prop), prop);
                        proxies[prop] = proxy;
                        return proxy;
                    }
                }
